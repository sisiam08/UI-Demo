...
export default function ProfileView({
  ...
  const photoSrc = !photoError ? loadPhoto(profile.photoUrl) : null;
  ...
  return (
    <div className="max-w-2xl space-y-4">
          ...
          <div className="-mt-12 mb-4 flex items-end justify-between gap-3">
            {isOwnProfile ? (
              <button
                ...
                className="group relative shrink-0 rounded-full ring-4 ring-card"
              >
                {avatar}
          ...
          {profile.bio ? (
            ...
            isOwnProfile && (
              <button
                type="button"
                onClick={() => setEditing(true)}
                className="mt-4 flex w-full items-center gap-1.5 border-t border-border pt-4 text-left text-sm text-muted-foreground italic hover:text-foreground"
              >
          ...
          <CardContent>
            {profile.skills.length > 0 ? (
              ...
              <button
                type="button"
                onClick={() => setEditing(true)}
                className="flex items-center gap-1.5 text-sm text-muted-foreground italic hover:text-foreground"
              >
                <Plus className="size-3.5" />
                Add skills so others know what you bring
              </button>
            )}
          </CardContent>
        </Card>
      ...
      {(profile.interestedIndustries.length > 0 || isOwnProfile) && (
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center gap-2 text-sm font-semibold">
